<template>
  <div class="container mx-auto p-6">
    <h1 class="text-2xl font-semibold mb-6 text-slate-700">分类管理</h1>

    <div class="mb-6">
      <button
        @click="openAddModal"
        class="inline-flex items-center bg-blue-600 hover:bg-blue-700 text-white font-semibold py-2 px-4 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-150"
      >
    ...
    <!-- Categories Table -->
    <div v-if="!isLoading && categories.length > 0" class="bg-white shadow-md rounded-lg overflow-hidden">
      <table class="min-w-full divide-y divide-slate-200">
        <thead class="bg-slate-50">
          <tr>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">名称</th>
            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">描述</th>
        ...
        <tbody class="bg-white divide-y divide-slate-200">
          <tr v-for="category in categories" :key="category.id" class="hover:bg-slate-50 transition-colors duration-150">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <span :class="category.is_enabled ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'" class="px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full">
                {{ category.is_enabled ? '启用' : '禁用' }}
              </span>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
              <button @click="openEditModal(category)" class="text-indigo-600 hover:text-indigo-800 transition-colors duration-150 inline-flex items-center">
              ...
              <button
                ...
                class="transition-colors duration-150 inline-flex items-center"
              >
                <svg v-if="category.is_enabled" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div v-if="!isLoading && categories.length === 0" class="text-center py-12">
      ...
      <p class="mt-3 text-xl font-medium text-slate-500">暂无分类信息</p>
      <p class="mt-1 text-sm text-slate-400">您可以点击"添加新分类"按钮来创建第一个分类。</p>
    </div>
...
const closeModal = () => {
  ...
  // Do not reset currentCategory here if you want to keep data on brief close,
  // or reset it if you prefer a clean slate every time.
  // currentCategory.value = { id: null, name: '', description: '', is_enabled: true };
};
...
const handleSubmit = async () => {
  ...
  const originalIsLoading = isLoading.value; // Store original loading state
  isLoading.value = true; // Set loading for submit operation
